...
public class JavassistClient {
    ...
    private static void createProxy() throws Exception {
        ...
        CtClass cc = pool.makeClass("com.asiainfo.chapter1.luomin.pattern.proxy.demo1.StationProxy");

        //设置接口
        CtClass interface1 = pool.get("com.asiainfo.chapter1.luomin.pattern.proxy.demo1.TicketService");
        cc.setInterfaces(new CtClass[]{interface1});

        //设置Field
        CtField field = CtField.make("private com.asiainfo.chapter1.luomin.pattern.proxy.demo1.Station station;", cc);

        cc.addField(field);

        CtClass stationClass = pool.get("com.asiainfo.chapter1.luomin.pattern.proxy.demo1.Station");
