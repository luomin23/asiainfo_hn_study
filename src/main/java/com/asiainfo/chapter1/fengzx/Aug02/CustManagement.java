...
public class CustManagement {
    ...
    public void modify() {
        ...
        do {
            if (k >= custNo.length)
                break;
            if (custNo[k] == i) {
                System.out.println((new StringBuilder()).append(custNo[k]).append("\t\t").append(custBirth[k]).append("\t\t").append(custScore[k]).toString());
                j = k;
                break;
            }
            k++;
    ...
    public void show() {
        ...
        for (int j = 0; j < i && custNo[j] != 0; j++)
            System.out.println((new StringBuilder()).append(custNo[j]).append("\t\t").append(custBirth[j]).append("\t\t").append(custScore[j]).toString());
