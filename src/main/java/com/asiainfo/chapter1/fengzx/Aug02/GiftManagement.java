...
public class GiftManagement {
    ...
    public void sendLuckyCust() {
        ...
        if (scanner.next().equals("y")) {
            ...
            if (flag)
                System.out.println((new StringBuilder()).append("幸运客户获赠MP3：").append(s).toString());
            else
    ...
    public void sendGoldenCust() {
        ...
        System.out.println((new StringBuilder()).append("具有最高积分的会员是： ").append(custNo[i]).append("\t").append(custBirth[i]).append("\t").append(custScore[i]).toString());
        Gift gift = new Gift();
        gift.name = "苹果笔记本电脑";
        gift.price = 12000D;
